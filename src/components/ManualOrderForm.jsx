...
export default function ManualOrderForm({ onOrderCreated, onCancel }) {
  ...
  const totalAmount = items.reduce((sum, i) => sum + i.unit_price_cents * i.quantity, 0)
        ...
        <div className="mt-6 rounded-[24px] border border-white/70 bg-white/90 p-4 shadow-inner shadow-gray-50">
          ...
          {items.length === 0 ? (
            ...
            <ul className="mt-4 space-y-3">
              {items.map((item) => (
                <li key={item.product_id} className="flex items-center justify-between rounded-2xl border border-gray-100 bg-gray-50/80 px-3 py-2 text-sm text-gray-700">
                  <div>
                    <p className="font-semibold text-gray-900">{item.name}</p>
                    <p className="text-xs text-gray-500">{item.quantity} uds · {formatPrice(item.unit_price_cents)}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    <span className="font-semibold text-gray-900">{formatPrice(item.unit_price_cents * item.quantity)}</span>
                    <button
